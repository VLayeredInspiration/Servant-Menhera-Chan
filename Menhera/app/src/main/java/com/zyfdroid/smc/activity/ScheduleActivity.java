...
Vibrator vibrator;
	...
	public void onPrepareUi()
	{
		...
		if(null!=alarms&&alarms.length!=0){
			...
			//i.getExtras().putBoolean("check",true);
			...
			//if(true)throw new UnsupportedOperationException("出现了不可能出现的情况，肯定是有什么地方不对");
			processed=true;
			finishAndRemoveTask();
		}
...
boolean canAction=false;
	...
	public void onFrame()
	{
		if((System.currentTimeMillis()-alarmTime)>55000l){
			if(!isCheck()){
				//Main.e(new Exception(""+getIntent().getExtras().getBoolean("check",false)));
			dismissedAlarm();
			}
		}
	...
	public void dismissedAlarm(){
		...
		Main.e(new Exception("getStack"));
		}catch(Exception e){Main.e(e);}
		//if(true)throw new AndroidRuntimeException("make a breakpoint");
		finishAndRemoveTask();
	}
	...
	void resetCheck(){savestate("sche_ischk","");}
	void raiseChk(){
