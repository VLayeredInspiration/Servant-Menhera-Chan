...
public class GuideActivity extends BaseActivity {
    ...
    String[] querySafeAppInfo() {
        ...
        Set<String> allowPackages = new HashSet<String>();
    ...
    boolean isSafeApp(ApplicationInfo api, PackageManager pm) {
        try {
            ...
            for (int i = 0; i < appnameKw.length; i++) {
                if (appname.contains(appnameKw[i])) {
                    return true;
                }
            }
            for (int i = 0; i < pkgnameKw.length; i++) {
                if (pkgname.contains(pkgnameKw[i])) {
                    return true;
                }
            }
        } catch (Exception e) {
