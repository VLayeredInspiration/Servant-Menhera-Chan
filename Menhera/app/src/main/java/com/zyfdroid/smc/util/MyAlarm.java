...
public class MyAlarm {
    ...
    public static MyAlarm WeeklyDayAlarm(int ahour, int aminute, boolean[] days) {
        ...
        if (days.length != 7) {
            ...
            if (!legal) {
                throw new IllegalStateException("No Days Set Yet");
            }
        }
    ...
    public long nextTime() {
        ...
        switch (type) {
            case TYPE_ALARM_ONCE:
                ...
                cld.add(Calendar.MINUTE, delays * retryInterval);
    ...
    private long nextIntervalAlarmTime(long t, int h, int m, int i, int o, int d) {
        ...
        cld2.set(Calendar.MINUTE, m);
        cld2.set(Calendar.SECOND, 0);
        cld2.set(Calendar.MILLISECOND, 0);
        while (cld2.getTimeInMillis() <= (t)) {
            cld2.add(Calendar.DAY_OF_MONTH, i);
            cld2.get(Calendar.DAY_OF_YEAR);
            System.out.println(cld2.getTimeInMillis());
        }
        cld2.add(Calendar.MINUTE, retryInterval * d);
    ...
    public long nextWeekAlarmTime(Calendar cld, Date date) {
        ...
        //计算开始
        cld.setTimeInMillis(System.currentTimeMillis());
        cld.set(Calendar.HOUR_OF_DAY, hour);//千万不要用cld.HOUR，上午下午不分的，害我调试了好久
        cld.set(Calendar.MINUTE, minute);//首先设置好时间
        cld.add(Calendar.MINUTE, retryInterval * delays);
        ...
        while (!matchTheNeed) {
            ...
            result = result && week[cld.get(Calendar.DAY_OF_WEEK) - 1];//周判断
    ...
    public static MyAlarm loadAlarm(Context ctx, long alarmId) {
        try {
            return MyAlarm.fromString(ctx.getSharedPreferences("alarms", Context.MODE_PRIVATE).getString(String.valueOf(alarmId), ""));
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public static long[] getAllAlarm(Context ctx) {
        SharedPreferences shp = ctx.getSharedPreferences("alarms", Context.MODE_PRIVATE);
